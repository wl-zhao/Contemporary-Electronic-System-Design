...
static tLoggerState g_sLoggerState = STATE_VIEWING;
...
extern union recordBuf g_sRecordBuf;
...
//            AcquireStop();
            ...
            (pWidget == &g_sTempContainerCanvas.sBase))
    {
        ...
        if(bActivated)
        {
            if (pWidget == &g_sAccelContainerCanvas.sBase)
            {
            	UartFlag |= 0x8;
            	UartFlag &= 0xfe;
            	UARTSend((unsigned char *)"Type: Accel\r\n", 13);
            }
            else if (pWidget == &g_sTempContainerCanvas.sBase)
            {
            	UartFlag |= 0x9;
            	UARTSend((unsigned char *)"Type: Temp\r\n", 12);
            }
...
//
//*****************************************************************************
short string2short(char* str)
{
	short ans = (str[0] - '0') * 10 + (str[1] - '0');
	return ans;
}
...
//                                   ROM_UARTCharGetNonBlocking(UART0_BASE));
    	msg = ROM_UARTCharGetNonBlocking(UART0_BASE);
    	if (msg == (long)('r') || msg == (long)('R'))
    	{
    		//Request data
    		if (!(UartFlag & 0x8))//Unavailable
    			UARTSend((unsigned char*)"Please Choose The Logger Type\r\n", 31);
    		else
    		{
    			UartFlag ^= 0x4;//Start Updating
    			if ((UartFlag & 0x4) == 0x4)
    				UARTSend((unsigned char*)"Start Updating\r\n", 16);
    			else
    				UARTSend((unsigned char*)"Stop Updating\r\n", 15);
    		}
    	}
    	else if (msg == (long)('k') || msg == (long)('K'))
    	{
    		UartFlag ^= 0x2;
    		if (UartFlag & 0x2)
    			UARTSend((unsigned char *)"Keep updating\r\n", 15);
    		else
    			UARTSend((unsigned char *)"Update once\r\n", 13);
    	}
    	else if (msg == (long)('$'))
    	{
    		seting = 0;
    		setTempBuf[buf_count] = '\0';
    		temp_target = string2short(setTempBuf) * 10;
//    		sprintf(setTempBuf, "target:%d\r\n", temp_target / 10);
//    		UARTSend(setTempBuf, 11);
    		buf_count = 0;
    	}
    	else if (msg == (long)('s'))
    	{
    	...
    	{
    		setTempBuf[buf_count++] = msg;
    	}
    }

//    char str[] = "flag changed, Flag = ";
//    sprintf(str, "%s%x", str, UartFlag);
//    UARTSend((unsigned char *)str, strlen(str));
}
...
void PWMInit()
{
	...
	ROM_PWMPulseWidthSet(PWM_BASE, PWM_OUT_1, 5999);
	// 使能PWM2和PWM3的输出
...
{
	if (q > 0)
		ROM_PWMPulseWidthSet(PWM_BASE, PWM_OUT_0, q * 60 - 1);
	else
...
//            g_sLoggerState = STATE_LOGGING;
    ...
    while(1)
    {
        ...
        //
        WidgetMessageQueueProcess();
        temp_now = g_sRecordBuf.sRecord.sItems[LOG_ITEM_EXTTEMP];
//        sprintf(buf, "now: %d\n", temp_now/10);
//        UARTSend(buf, strlen(buf));
        if (temp_now < temp_target)
        {
        	if (q < 100)
        	{
        		q += 1;
