...
// Flag to record the command from UART
// xxxxx000
// d0 = 0: ACCEL			d0 = 1: TEMP
// d1 = 0: Update once		d1 = 1: Keep updating
// d2 = 0: Stop Updating	d2 = 1: Start Updating
// d3 = 0: Unavailable		d3 = 1: Available
// initial state: 00000 0000
...
//    sprintf(cTip, "Flag = %x\r\n", UartFlag);
//    UARTSend((unsigned char *)cTip, strlen(cTip));
    unsigned long ulIdx;
    tTime sTime;
    unsigned long ulRTC;
    ...
    for(ulIdx = LOG_ITEM_ACCELX; ulIdx <= LOG_ITEM_ACCELZ; ulIdx++)
    {
        ...
        MenuUpdateText(ulIdx, cViewerBuf);
        if (((UartFlag & 0xc) == 0xc) && ((UartFlag & 0x1) == 0x0))//Available, updating, acceleration
        {
        	UARTSend((unsigned char *)cViewerBuf, 11);
        	UARTSend((unsigned char *)"    ", 4);
        }
    }
    //
    ...
    if (((UartFlag & 0xc) == 0xc) && ((UartFlag & 0x1) == 0x1))//Available, updating, temp
    {
    	UARTSend((unsigned char *)cViewerBuf, 13);
    	UARTSend((unsigned char *)"    ", 4);
    }
    ...
    if (((UartFlag & 0xc) == 0xc) && ((UartFlag & 0x1) == 0x1))
    {
//    	usnprintf(cUartBuf + sizeof(cViewerBuf) + 2, sizeof(cViewerBuf) + 2, "%s\r\n", cViewerBuf);
    	UARTSend((unsigned char *)cViewerBuf, 13);
    	UARTSend((unsigned char *)"    ", 4);
    }
...
//    // Send to Uart
//    if (UartFlag & 0x12)
//    	UARTSend((unsigned char *)cUartBuf, sizeof(cUartBuf));
    // if Update once, Reset
    if ((UartFlag & 0xc) == 0xc)
    	UARTSend("\r\n", 2);
    if ((UartFlag & 0x6) == 0x4)
    	UartFlag &= 0xfb;
...
//        UARTSend((unsigned char *)"love lucy\r\n", 11);
        //
